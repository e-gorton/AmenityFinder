...
- Overpass mirrors are hedged for resilience. A successful but empty response cannot cancel other mirrors that are still returning real OSM features; an empty local result is accepted only when every successful mirror agrees.
