...
test("screens low-confidence, inactive and duplicate amenity records", async () => {
  ...
  assert.match(route, /SPARSE_COMMERCIAL_MAX_AGE_MS = 10 \* 365\.25/);
  assert.match(route, /function isStaleSparseCommercialFeature/);
  assert.match(route, /function hasOperationalEvidence/);
  assert.match(route, /api\.ratings\.food\.gov\.uk/);
  assert.match(route, /fetchNearbyFsaEstablishments/);
  assert.match(route, /matchesFsaEstablishment/);
  assert.match(route, /Food Standards Agency register \(validation only\)/);
  assert.match(route, /fsaEstablishments === null/);
  assert.match(route, /out center meta;/);
  assert.match(route, /qualityFilteredCount/);
  assert.match(page, /Only results passing quality screening are shown/);
  assert.match(page, /not a live trading-status\s+register/);
  assert.match(page, /Validation_Status/);
  assert.match(page, /Data_Source/);
  assert.match(page, /Source_Updated/);
});
